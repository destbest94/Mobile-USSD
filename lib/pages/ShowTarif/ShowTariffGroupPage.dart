...
import 'package:mobile_ussd/pages/ShowTarif/components/tariffGroups.dart';

class ShowTariffGroupPage extends StatefulWidget {
  final dynamic model;
  final String title;
  ...
  @override
  _ShowTariffGroupPageState createState() => _ShowTariffGroupPageState();
}

class _ShowTariffGroupPageState extends State<ShowTariffGroupPage> {
  List tabs() {
    List<Widget> list = [];
  ...
  Future<List<Widget>> tabBarView() async {
    List<TariffPlanModel> packageModels = await ConnectToServer().getTariffPlan(
        mobileOperator: widget.mobileOperator, groupsId: getGroupsId());

    if (packageModels.length == 0) {
      packageModels = await ConnectToServer()
          .getTariffPlan(mobileOperator: '', groupsId: getGroupsId());
    }
