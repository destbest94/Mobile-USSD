...
//url: http://ussd01.thelab.uz/api/ussd-codes-group/

class UssdCodesGroupController {
  Database _db;
  final String _tableName = 'ussd_codes_group';
  ...
  Future<List<UssdCodesGroupModel>> majorOperation(
      {String mobileOperator,
      String language,
      List<dynamic> decodedJsonList}) async {
    _db = await database();
    UssdCodesGroupModel model;

    if (decodedJsonList.length > 0) {
      for (var m in decodedJsonList) {
        model = UssdCodesGroupModel.fromJson(m);
  ...
  Future<List<UssdCodesGroupModel>> _getAll(
    ...
    return List.generate(
        maps.length, (i) => UssdCodesGroupModel.fromJson(maps[i]));
  }

  Future<void> _inser(UssdCodesGroupModel model) async {
    await _db.insert('$_tableName', model.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<int> _update(UssdCodesGroupModel model) async {
    return await _db.update('$_tableName', model.toMap(),
        where: 'id = ?', whereArgs: [model.id]);
  }

  Future<int> _delete(UssdCodesGroupModel model) async {
    return await _db
