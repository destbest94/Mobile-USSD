...
//url: http://ussd01.thelab.uz/api/minute-packages-group/

class MinutePackagesGroupController {
  Database _db;
  final String _tableName = 'minute_packages_group';
  ...
  Future<List<MinutePackagesGroupModel>> majorOperation(
      {String mobileOperator,
      String language,
      List<dynamic> decodedJsonList}) async {
    _db = await database();
    MinutePackagesGroupModel model;

    if (decodedJsonList.length > 0) {
      for (var m in decodedJsonList) {
        model = MinutePackagesGroupModel.fromJson(m);
  ...
  Future<List<MinutePackagesGroupModel>> _getAll(
    ...
    return List.generate(
        maps.length, (i) => MinutePackagesGroupModel.fromJson(maps[i]));
  }

  Future<void> _inser(MinutePackagesGroupModel model) async {
    await _db.insert('$_tableName', model.toMap(),
        conflictAlgorithm: ConflictAlgorithm.replace);
  }

  Future<int> _update(MinutePackagesGroupModel model) async {
    return await _db.update('$_tableName', model.toMap(),
        where: 'id = ?', whereArgs: [model.id]);
  }

  Future<int> _delete(MinutePackagesGroupModel model) async {
    return await _db
